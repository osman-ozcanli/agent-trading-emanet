...
Tanıklardan biri ulaşılamazsa `unknown` döner — **çökmez, ama işlemi de onaylamaz.**
...
RISK    src/risk.py           saf fonksiyon, aga cikmaz, VETO yetkili — tek "hayir" diyebilen katman
   │
MCP     src/mcp.py            JSON-RPC/stdio istemcisi, 130 satir, sifir bagimlilik, 168 ATK araci
   │
...
## 3. Risk kuralları (`config.yaml` — kodda sabit değer yok)
...
| Günlük kayıp | %3 → Safe Mode (kalıcı) | "Safe Mode aktif" |
...
pip install pyyaml
python src/agent.py --once      # tek tur
python src/agent.py             # otonom dongu
python src/dashboard.py         # http://localhost:8787
```
...
- **Demo:** `logs/journal.jsonl` — 17:36'dan itibaren temiz defter (öncesi karantinada, bkz. §7)
- **Canlı:** `logs/journal.live.jsonl` — 18:29–18:30, 2 gözetimli tur, 4 gerçek emir, 5 red.
  Borsadan bağımsız doğrulama: 4 OCO `live`, 4 fill, bakiye 30 → 20,40 kuruşuna kadar tutarlı.
...
- **Kâr-al/zarar-kes'in fiyatla tetiklenmesi bugün gözlemlenmedi.** Emirler borsada `live` duruyor;
  test penceresinde fiyat bantlara girmedi. Zaman stopu ise 3 kez tetiklendi ve pozisyon kapattı (fill kanıtlı).
...
## 8. Bugün yaşanan ve düzeltilen 6 hata

Hepsi demo'da yakalandı, hiçbiri gerçek parayla öğrenilmedi. Detay `PROGRESS.md`'de.
...
src/okx.py         CLI yedek yolu       src/llm.py        terk edilen LLM katmani (kapali)
